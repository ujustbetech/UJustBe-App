...
- (void)_updateFrame {
    ...
    switch (self.keyboardResizes) {
        ...
        case ResizeNative: {
            [self.webView setFrame:CGRectMake(wf.origin.x, wf.origin.y, f.size.width - wf.origin.x,
                                              f.size.height - wf.origin.y - self.paddingBottom)];
            break;
        }
        default:
            break;
...
- (void)setHideFormAccessoryBar:(BOOL)hideFormAccessoryBar {
    ...
    Method UIMethod = class_getInstanceMethod(NSClassFromString(UIClassString),
