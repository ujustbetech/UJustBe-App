...
- (void)setSb_hideStatusBar:(id)newHideStatusBar {
    objc_setAssociatedObject(self, kHideStatusBar, newHideStatusBar,
...
- (void)setSb_statusBarStyle:(id)newStatusBarStyle {
    objc_setAssociatedObject(self, kStatusBarStyle, newStatusBarStyle,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    ...
    [self.sb_statusBarStyle intValue];
}
...
- (void)pluginInitialize {
    ...
    _uiviewControllerBasedStatusBarAppearance = (uiviewControllerBasedStatusBarAppearance == nil ||
                                                 [uiviewControllerBasedStatusBarAppearance boolValue]);
    ...
    if ([self settingForKey:setting]) {
        self.webView.scrollView.scrollsToTop = [(NSNumber * )[
        self
        settingForKey:
        setting] boolValue];
    } else {
    ...
    fakeScrollView.contentSize = CGSizeMake(UIScreen.mainScreen.bounds.size.width,
                                            UIScreen.mainScreen.bounds.size.height *
                                            2.0f); // Make the scroll view longer than the screen itself
    fakeScrollView.contentOffset = CGPointMake(0.0f,
                                               UIScreen.mainScreen.bounds.size.height); // Scroll down so a tap will take scroll view back to the top
...
- (void)_ready:(CDVInvokedUrlCommand *)command {
    ...
    if ([self settingForKey:setting]) {
        self.statusBarOverlaysWebView = [(NSNumber * )[
        self
        settingForKey:
        setting] boolValue];
...
- (void)initializeStatusBarBackgroundView {
    ...
    if ([[UIApplication sharedApplication] statusBarOrientation] ==
        ...
        statusBarFrame.size.height + statusBarFrame.origin.y ==
        [self.viewController.view.window bounds].size.height) {
...
- (void)_backgroundColorByHexString:(NSString *)hexString {
    ...
    _statusBarBackgroundColor = [UIColor colorWithRed:((rgbValue & 0xFF0000) >> 16) / 255.0 green:
