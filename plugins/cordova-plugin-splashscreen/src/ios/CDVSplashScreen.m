...
- (UIInterfaceOrientation)getCurrentOrientation {
    ...
    if (dOrientation == UIDeviceOrientationUnknown || dOrientation == UIDeviceOrientationFaceUp ||
...
- (void)setVisible:(BOOL)visible andForce:(BOOL)force {
    if (visible != _visible || force) {
        ...
        float fadeDuration = fadeSplashScreenDuration == nil ? kFadeDurationDefault
                                                             : [fadeSplashScreenDuration floatValue];
        ...
        if (_visible) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                                         (uint64_t) effectiveSplashDuration * NSEC_PER_SEC),
