...
- (id)init {
    ...
    IFD0TagFormatDict = [[NSDictionary alloc] initWithObjectsAndKeys:
            ...
            TAGINF_W_APPEND(@"0132", [NSNumber numberWithInt:EDT_ASCII_STRING], @20,
                            @"00"), @"DateTime",
    ...
    SubIFDTagFormatDict = [[NSDictionary alloc] initWithObjectsAndKeys:
            ...
            TAGINF_W_APPEND(@"9004", [NSNumber numberWithInt:EDT_ASCII_STRING], @20,
                            @"00"), @"DateTimeDigitized",
            TAGINF_W_APPEND(@"9003", [NSNumber numberWithInt:EDT_ASCII_STRING], @20,
                            @"00"), @"DateTimeOriginal",
...
- (NSData *)spliceExifBlockIntoJpeg:(NSData *)jpegdata withExifBlock:(NSString *)exifstr {
    ...
    while (loc + 1 < [jpegdata length]) {
        ...
        if ([[blag description] isEqualToString:@"<ffe1>"]) {
            ...
            NSString *the = [exifWriter hexStringFromData:[jpegdata subdataWithRange:NSMakeRange(
                    loc + 2, 2)]];
...
- (NSString *)createExifAPP1:(NSDictionary *)datadict {
    ...
    */
    ...
    app1 = [[NSMutableString alloc] initWithFormat:@"%@%04x%@%@%@%@%@",
                                                   app1marker,
                                                   (unsigned int) (16 + ([exifIFD length] / 2) +
                                                                   ([subExifIFD length] /
                                                                    2)) /*16+[exifIFD length]/2*/,
                                                   exifmarker,
                                                   tiffheader,
                                                   ifd0offset,
                                                   exifIFD,
...
- (NSString *)createExifIFDFromDict:(NSDictionary *)datadict
    ...
    if (ifd0flag) {
        ...
        NSNumber *offset = [NSNumber numberWithUnsignedInteger:[exifstr length] / 2 +
                                                               [dbstr length] / 2 + 18 + 8];
...
- (NSString *)createIFDElement:(NSString *)elementName withFormat:(NSArray *)formtemplate withElementData:(NSString *)data {
    ...
    if (formtemplate) {
        ...
        if ([components intValue] == 0) {
            components = [NSNumber numberWithUnsignedInteger:[data length] *
                                                             DataTypeToWidth[[dataformat intValue] -
                                                                             1]];
        }
...
- (NSString *)createIFDElementDataWithFormat:(NSArray *)dataformat withData:(NSString *)data {
    ...
    switch ([formatcode intValue]) {
        ...
        case EDT_ASCII_STRING:
            ...
            if ([datastr length] < 8) {
                NSString *format = [NSString stringWithFormat:@"%%0%dd", (int) (8 -
                                                                                [datastr length])];
...
- (NSString *)formatRationalWithNumerator:(NSNumber *)numerator withDenominator:(NSNumber *)denominator asSigned:(Boolean)signedFlag {
    ...
    if (signedFlag) {
        ...
        [str appendFormat:@"%08lx%08lx",
                          num >= 0 ? num : ~ABS(num) + 1, num >= 0 ? den : ~ABS(den) + 1];
    } else {
