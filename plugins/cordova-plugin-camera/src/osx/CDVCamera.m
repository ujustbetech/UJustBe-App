...
- (void)takePictureFromCamera:(CDVInvokedUrlCommand *)command withOptions:(CDVPictureOptions *)pictureOptions {
    ...
    [pictureTaker beginPictureTakerSheetForWindow:self.viewController.contentView.window withDelegate:self didEndSelector:@selector(pictureTakerDidEnd:returnCode:contextInfo:) contextInfo:(void *) CFBridgingRetain(
            contextInfo)];
...
- (void)takePictureFromFile:(CDVInvokedUrlCommand *)command withOptions:(CDVPictureOptions *)pictureOptions {
    ...
    [openPanel beginSheetModalForWindow:self.viewController.contentView.window completionHandler:^(
            NSInteger result) {
...
- (NSData *)processImage:(NSImage *)image options:(CDVPictureOptions *)pictureOptions {
    ...
    NSData *data = (pictureOptions.encodingType == EncodingTypeJPEG)
                   ? [imageRepresentation representationUsingType:NSJPEGFileType properties:@{
                    NSImageCompressionFactor: [NSNumber numberWithFloat:
                            pictureOptions.quality.floatValue / 100.f]}]
                   : [imageRepresentation representationUsingType:NSPNGFileType properties:@{
