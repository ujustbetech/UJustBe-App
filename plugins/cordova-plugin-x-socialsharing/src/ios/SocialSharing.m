...
- (void)shareInternal:(CDVInvokedUrlCommand *)command withOptions:(NSDictionary *)options isBooleanResponse:(BOOL)boolResponse {
    [self.commandDelegate runInBackground:^{ //avoid main thread block  especially if sharing big files from url
        ...
        if ([activityVC respondsToSelector:(@selector(setCompletionWithItemsHandler:))]) {
            [activityVC setCompletionWithItemsHandler:^(NSString *activityType, BOOL completed,
                ...
                if (boolResponse) {
                    ...
                    NSDictionary *result = @{@"completed": @(completed), @"app": activityType == nil
                                                                                 ? @""
                                                                                 : activityType};
            ...
            if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
                ...
                if (iPadCoords != nil && ![iPadCoords isEqual:@"-1,-1,-1,-1"]) {
                    ...
                    if ([iPadCoordinates count] == 4) {
                        ...
                        rect = CGRectMake((int) [[iPadCoordinates objectAtIndex:0] integerValue],
                                          (int) [[iPadCoordinates objectAtIndex:1] integerValue],
                                          (int) [[iPadCoordinates objectAtIndex:2] integerValue],
                                          (int) [[iPadCoordinates objectAtIndex:3] integerValue]);
                    } else {
    ...
    if (message != (id) [NSNull null]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1000 * NSEC_PER_MSEC),
                    ...
                    if (fbAppInstalled) {
                        ...
                        [alert show];
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 2800 * NSEC_PER_MSEC),
                                       dispatch_get_main_queue(), ^{
...
- (void)canShareVia:(CDVInvokedUrlCommand *)command {
    ...
    if ([@"sms" caseInsensitiveCompare:via] == NSOrderedSame && [self canShareViaSMS]) {
        ...
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    } else if ([@"whatsapp" caseInsensitiveCompare:via] == NSOrderedSame &&
               [self canShareViaWhatsApp]) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    } else if ([@"instagram" caseInsensitiveCompare:via] == NSOrderedSame &&
               [self canShareViaInstagram]) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    } else if ([@"com.apple.social.facebook" caseInsensitiveCompare:via] == NSOrderedSame &&
               [self canShareViaFacebook]) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    } else if ([@"com.apple.social.twitter" caseInsensitiveCompare:via] == NSOrderedSame &&
               [self canShareViaTwitter]) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    ...
    [composeViewController setCompletionHandler:^(SLComposeViewControllerResult result) {
        if (SLComposeViewControllerResultCancelled == result) {
            ...
            [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
                   ...
                   [self isAvailableForSharing:command type:type]) {
...
- (void)shareViaEmail:(CDVInvokedUrlCommand *)command {
    if ([self isEmailAvailable]) {
        ...
        if ([command.arguments objectAtIndex:0] != (id) [NSNull null]) {
            NSString *message = [command.arguments objectAtIndex:0];
            BOOL isHTML =
                    [message rangeOfString:@"<[^>]+>" options:NSRegularExpressionSearch].location !=
                    NSNotFound;
        ...
        if ([command.arguments objectAtIndex:5] != (id) [NSNull null]) {
            ...
            for (NSString *path in attachments) {
                ...
                if (rangeData.location == NSNotFound) {
                    ...
                    mimeType = (NSString *) [[[basename substringFromIndex:rangeData.location +
                                                                           rangeData.length] componentsSeparatedByString:@";"] objectAtIndex:0];
                    ...
                    if (rangeDF.location == NSNotFound) {
                        ...
                        //Found, apply name
                        fileName = (NSString *) [[[basename substringFromIndex:rangeDF.location +
                                                                               rangeDF.length] componentsSeparatedByString:@";"] objectAtIndex:0];
                    }
...
- (NSString *)getMimeTypeFromFileExtension:(NSString *)extension {
    ...
    CFStringRef type = UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension, ext,
                                                             NULL);
        ...
        for (NSString *filename in filenames) {
            ...
            if (image != nil) {
                ...
                UIImageWriteToSavedPhotosAlbum(image, self,
                                               @selector(thisImage:wasSavedToPhotoAlbumWithError:contextInfo:),
                                               nil);
            }
        }
        if (!shared) {
...
- (NSURL *)getFile:(NSString *)fileName {
    ...
    if (fileName != (id) [NSNull null]) {
        ...
        if ([fileName hasPrefix:@"http"]) {
            ...
            //If found "data:"
            NSString *fileType = (NSString *) [[[fileName substringFromIndex:rangeData.location +
                                                                             rangeData.length] componentsSeparatedByString:@";"] objectAtIndex:0];
            ...
            if (rangeDF.location == NSNotFound) {
                ...
                //Found, apply name
                attachmentName = (NSString *) [[[fileName substringFromIndex:rangeDF.location +
                                                                             rangeDF.length] componentsSeparatedByString:@";"] objectAtIndex:0];
            }
